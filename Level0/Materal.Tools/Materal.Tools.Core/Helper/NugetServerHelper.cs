    ...
    public static partial class NugetServerHelper
    {
        ...
        public static async Task UploadNugetPackagesAsync()
        {
            ...
            foreach (FileInfo nugetFileInfo in fileInfos)
            {
                ...
                {
                    ConsoleQueue.WriteLine("等待Nuget服务器清理缓存...");
                    await Task.Delay(1000);
                }
                ConsoleQueue.WriteLine($"Nuget服务器已检测到包{nugetFileInfo.Name}");
            }
        }
        /// <summary>
        /// 上传Nuget包
        ...
        {
            ConsoleQueue.WriteLine($"开始上传{nugetFileInfo.Name}到服务器...");
...
#pragma warning restore SYSLIB0014 // 类型或成员已过时
            ...
            strm.Close();
            fs.Close();
            ConsoleQueue.WriteLine($"{nugetFileInfo.Name}上传成功...");
        }
