...
namespace Materal.BusinessFlow.ADONETRepository
{
    public abstract class BaseUnitOfWorkImpl : IUnitOfWork
    {
        ...
        private readonly List<Func<IDbCommand>> commands = new();
        ...
        public virtual void Commit()
        {
            ...
            try
            {
                foreach (Func<IDbCommand> command in commands)
                {
                    IDbCommand sqliteCommand = command.Invoke();
        ...
        public virtual void RegisterAdd<T>(T obj)
            where T : class, IBaseDomain => commands.Add(() => GetInsertDomainCommand(_connection, obj));
        public virtual void RegisterDelete<T>(T obj)
            where T : class, IBaseDomain => commands.Add(() => GetDeleteDomainCommand(_connection, obj));
        public virtual void RegisterEdit<T>(T obj)
            where T : class, IBaseDomain => commands.Add(() => GetEditDomainCommand(_connection, obj));
        /// <summary>
        /// 注册命令
        /// </summary>
        /// <param name="addFunc"></param>
        public virtual void RegisterCommand(Func<IDbConnection, IDbCommand> addFunc) => commands.Add(() => addFunc(_connection));
