    ...
    public class LoggerTest
    {
        ...
        private Func<Action<LoggerConfigOptions>, Task> _initAsync = async action =>
        {
            ...
            serviceCollection.AddMateralLogger(option =>
            {
                ...
                option.SetLoggerLogLevel(LogLevel.Information);
            });
            IServiceProvider services = serviceCollection.BuildServiceProvider();
            await WriteLogAsync(services);
        };
        /// <summary>
        /// 构造方法
        /// </summary>
        ...
        public async Task WriteConsoleLogTestAsync() 
            => await _initAsync(option => option.AddConsoleTarget("ConsoleLogger", _textFormat));
        /// <summary>
        /// 写文件日志
        /// </summary>
        /// <returns></returns>
        ...
        public async Task WriteFileLogTestAsync()
            => await _initAsync(option => option.AddFileTarget("FileLogger", "Logs/${Level}.log", _textFormat));
        /// <summary>
        /// 写Http日志
        /// </summary>
        /// <returns></returns>
        ...
        public async Task WriteHttpLogTestAsync()
            => await _initAsync(option => option.AddHttpTarget("HttpLogger", "http://localhost:5000/api/Log/Write${Level}", HttpMethod.Post));
        /// <summary>
        /// 写日志
        /// </summary>
