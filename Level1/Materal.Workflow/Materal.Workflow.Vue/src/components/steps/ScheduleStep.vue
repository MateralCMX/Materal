...
<template>
    <div :id=stepID ref="stepElement" class="Step ScheduleStep" @dblclick="OpenEditModal()" :title="stepModel?.StepData?.Name">
        {{ stepModel?.StepData?.Name }}
        <div class="Point NextPoint" title="下一步"></div>
        <div class="Point StepPoint" title="计划节点"></div>
...
import { ScheduleStepData } from '../../scripts/StepDatas/ScheduleStepData';
import { IStep } from '../../scripts/IStep';

const emits = defineEmits<{
    (event: "showStepEditModal", stepModel: ScheduleStepModel): void
}>();
const props = defineProps<{ stepID: string, instance?: BrowserJsPlumbInstance }>();
const exposeModel: IStep<ScheduleStepModel, ScheduleStepData> = {
    GetStepModel: (): ScheduleStepModel | undefined => toRaw(stepModel.value),
...
const stepModel = ref<ScheduleStepModel>();
watch(instance, m => {
    if (!m) return;
    InitPage(m);
});
...
const InitPage = (canvas: BrowserJsPlumbInstance) => {
    ...
    stepModel.value = new ScheduleStepModel(id.value, canvas, stepElement.value);
}
/**
 * 打开编辑模态框
 */
