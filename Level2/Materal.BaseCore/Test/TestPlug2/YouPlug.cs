...
namespace TestPlug2
{
    public class YouPlug : IMateralBaseCoreCodeGeneratorPlug
    {
        public void PlugExecute(DomainPlugModel model)
        {
            ...
            codeContent.AppendLine($"    public class YouPlug{model.Domain.Name}ShowName");
            codeContent.AppendLine($"    {{");
            ...
            codeContent.AppendLine($"    }}");
            codeContent.AppendLine($"}}");
            codeContent.SaveFile(model.DomainProject.GeneratorRootPath, $"YouPlug{model.Domain.Name}ShowName.g.cs");
        }
    }
}