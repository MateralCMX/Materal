...
namespace TestPlug
{
    public class MyPlug2 : IMateralBaseCoreCodeGeneratorPlug
    {
        public void PlugExecute(DomainPlugModel model)
        {
            ...
            codeContent.AppendLine($"    public class MyPlug{model.Domain.Name}ShowName2");
            codeContent.AppendLine($"    {{");
            ...
            codeContent.AppendLine($"    }}");
            codeContent.AppendLine($"}}");
            codeContent.SaveFile(model.DomainProject.GeneratorRootPath, $"MyPlug{model.Domain.Name}ShowName2.g.cs");
        }
    }
}