...
namespace Materal.BusinessFlow.SqliteRepository.Repositories
{
    public class FlowRepositoryImpl : BaseFlowRepositoryImpl, IFlowRepository
    {
        ...
        private List<DataModelField> GetDataModelFields(Guid dataModelID, IDbCommand sqliteCommand)
        {
            ...
            sqliteCommand.AddParameter(UnitOfWork.GetParams(nameof(DataModelField.DataModelID)), dataModelID);
...
WHERE {UnitOfWork.GetTSQLField(nameof(DataModelField.DataModelID))} = {UnitOfWork.GetParams(nameof(DataModelField.DataModelID))}";
            ...
            while (dr.Read())
            {
                ...
                DataModelField? domain = RepositoryHelper.DataReaderConvertToEntity<DataModelField>(dr);
