...
namespace Materal.BaseCore.CodeGenerator.Models
{
    public class DomainModel
    {
        ...
        public void CreateServiceImplFile(ProjectModel project)
        {
            ...
            codeContent.AppendLine($"using Materal.Utils.Enums;");
            ...
            if (IsTreeDomain)
            {
                ...
                string name = _queryTargetName ?? Name;
                string repositoryName = _queryTargetName is null ? "DefaultRepository" : "DefaultViewRepository";
                codeContent.AppendLine($"        /// <summary>");
                codeContent.AppendLine($"        /// 查询树列表");
                codeContent.AppendLine($"        /// </summary>");
                codeContent.AppendLine($"        /// <param name=\"queryModel\"></param>");
                ...
                codeContent.AppendLine($"            if ({repositoryName} is ICacheEFRepository<{name}, Guid> cacheRepository)");
                codeContent.AppendLine($"            {{");
                ...
                codeContent.AppendLine($"            else");
                codeContent.AppendLine($"            {{");
                codeContent.AppendLine($"                allInfo = await {repositoryName}.FindAsync(queryModel, sortExpression, sortOrder);");
                codeContent.AppendLine($"            }}");
                codeContent.AppendLine($"            #endregion");
