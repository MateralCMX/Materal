namespace Materal.MergeBlock.EventBus
{
    ...
    public class EventBusModule : MergeBlockModule, IMergeBlockModule
    {
        ...
        {
            List<Type> handlerTypes = GetAllEventHandler(MergeBlockHost.ModuleInfos);
        ...
        public override async Task OnConfigServiceAfterAsync(IConfigServiceContext context)
        {
            ...
            context.Services.AddSingleton<IEventBus, EventBusRabbitMQ>(serviceProvider =>
            {
                ...
                IEnumerable<IModuleInfo> moduleInfos = serviceProvider.GetServices<IModuleInfo>();
                List<Type> handlerTypes = GetAllEventHandler(MergeBlockHost.ModuleInfos);
                foreach (Type handlerType in handlerTypes)
                {
                    Type? eventHanlerInterfaceType = handlerType.GetAllInterfaces().FirstOrDefault(m => m.IsGenericType && m.GenericTypeArguments.Length == 1 && m.IsAssignableTo<IIntegrationEventHandler>());
        ...
        /// <returns></returns>
        private List<Type> GetAllEventHandler(IEnumerable<IModuleInfo> moduleInfos)
        {
            if (_eventHandlerTypes.Count > 0) return _eventHandlerTypes;
            foreach (IModuleInfo moduleInfo in moduleInfos)
            {
                IEnumerable<Type> types = moduleInfo.ModuleType.Assembly.GetTypes().Where(m => !m.IsAbstract && m.IsClass && m.IsAssignableTo<IIntegrationEventHandler>());
                _eventHandlerTypes.AddRange(types);
            }
