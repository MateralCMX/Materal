...
namespace Materal.MergeBlock.Filters
{
    ...
    /// 异常拦截器
    /// </summary>
    public class GlobalExceptionFilter(ILogger<GlobalExceptionFilter> logger, IOptionsMonitor<MergeBlockConfig> config) : IExceptionFilter
    {
        /// <summary>
        /// 自定义异常处理
        /// </summary>
        ...
        /// <returns></returns>
        private ResultModel GetDefaultResult(string message)
        {
            ResultModel result = config.CurrentValue.Exception.ShowException ? ResultModel.Fail(message) : ResultModel.Fail(config.CurrentValue.Exception.ErrorMessage);
            return result;
        }
        /// <summary>
        /// 写错误
        ...
        private async Task WriteErrorAsync(ExceptionContext context)
        {
            ...
            Exception exception = new MergeBlockException(message.ToString(), context.Exception);
