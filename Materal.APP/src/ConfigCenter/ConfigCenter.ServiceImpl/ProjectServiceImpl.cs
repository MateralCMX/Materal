...
namespace ConfigCenter.ServiceImpl
{
    public class ProjectServiceImpl : IProjectService
    {
        ...
        public ProjectServiceImpl(IProjectRepository projectRepository, IConfigCenterSqliteEFUnitOfWork configCenterSqliteEFUnitOfWork, IMapper mapper, INamespaceRepository namespaceRepository, IEventBus eventBus)
        {
        ...
        public async Task EditProjectAsync(EditProjectModel model)
        {
            ...
            string oldName = projectFromDb.Name;
            ...
            await _configCenterSqliteEFUnitOfWork.CommitAsync();
            EditProjectEvent @event = new EditProjectEvent
            {
                OldProjectName = oldName,
                NewProjectName = projectFromDb.Name,
            };
        ...
        public async Task DeleteProjectAsync([Required(ErrorMessage = "唯一标识不能为空")]Guid id)
        {
            ...
            await _configCenterSqliteEFUnitOfWork.CommitAsync();
            DeleteProjectEvent @event = new DeleteProjectEvent
            {
