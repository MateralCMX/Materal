...
namespace MateralBaseCoreVSIX.Models
{
    public class ControllerModel
    {
        ...
        private readonly List<string> _annotations = new List<string>();
        private bool _isServiceHttpClient = false;
        private string[] _tModels = new string[5];
        private List<ActionModel> actionModels = new List<ActionModel>();
        ...
        public void Append(string[] codes, int classLineIndex)
        {
            ...
            if (classCode.EndsWith(">"))
            {
                ...
                if (angleBracketStartIndex < 0) throw new VSIXException("类型错误");
        ...
        public void CreateHttpClientFile(ProjectModel project)
        {
            ...
            foreach (ActionModel action in actionModels)
            {
                ...
                if (!string.IsNullOrWhiteSpace(action.BodyParams))
                {
                    args.Add(action.BodyParams);
                    bodySendArgs = action.BodyParams.Split(' ').Last();
                }
                List<string> querySendArgs = new List<string>();
