...
namespace Materal.Gateway.Service
{
    ...
    /// Ocelot配置服务
    /// </summary>
    public class OcelotConfigServiceImpl(IOcelotConfigRepository ocelotConfigRepository, IConsulService? consulService = null) : IOcelotConfigService
    {
        /// <summary>
        /// 初始化
        /// </summary>
        ...
        {
            if (ocelotConfigRepository.OcelotConfig.Routes.Count == 1) throw new GatewayException("不能删除最后一个路由");
        ...
        {
            if (ocelotConfigRepository.OcelotConfig.SwaggerEndPoints.Count == 1) throw new GatewayException("不能删除最后一个配置");
        ...
        {
            if (consulService is null) throw new GatewayException("未配置Consul服务");
            ...
            if (servies.Count == 0) throw new GatewayException("未找到符合条件的服务");
