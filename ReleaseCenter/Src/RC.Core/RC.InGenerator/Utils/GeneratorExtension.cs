...
namespace RC.InGenerator.Utils
{
    public static class GeneratorExtension
    {
        ...
        /// <returns></returns>
        public static string GetAnnotation(this SyntaxNode syntaxNode)
        {
            var result = syntaxNode.GetLeadingTrivia().ToString();
            ...
            if (temp.Length > 2)
            {
                ...
                {
                    result = temp[i].Trim();
                    if(result == "/// <summary>" && i < temp.Length - 1)
                    {
                        result = temp[i + 1].Trim();
                        break;
                    }
                }
            }
        ...
        /// 获得特性组
        /// </summary>
        /// <param name="attributeListSyntaxs"></param>
        /// <returns></returns>
        public static List<AttributeModel> GetAttributeSyntaxes(this SyntaxList<AttributeListSyntax> attributeListSyntaxs)
        {
            List<AttributeModel> result = new();
            if (attributeListSyntaxs.Count <= 0) return result;
            foreach (AttributeListSyntax attributeListSyntax in attributeListSyntaxs)
            {
