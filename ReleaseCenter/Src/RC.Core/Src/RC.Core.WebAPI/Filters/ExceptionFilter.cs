...
namespace RC.Core.WebAPI.Filters
{
    /// <summary>
    /// 异常拦截器
    /// </summary>
    public class ExceptionFilter : IExceptionFilter
    {
        ...
        {
            if (exception is RCException || exception is AutoMapperMappingException aex && aex.InnerException != null && aex.InnerException is RCException)
            {
        ...
        {
            ResultModel result = RCConfig.ExceptionConfig.ShowException ? ResultModel.Fail(message) : ResultModel.Fail(RCConfig.ExceptionConfig.ErrorMessage);
            return result;
        }
        /// <summary>
        /// 写错误
